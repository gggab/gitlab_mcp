...
function Read-CursorConfig {
    ...
    try {
        ...
        throw "Cursor MCP configuration is not valid JSON: $ConfigPath"
    }
    if ($config -isnot [System.Management.Automation.PSCustomObject]) {
        throw "Cursor MCP configuration must be a JSON object: $ConfigPath"
    }
    return $config
}

function Set-CursorMcpConfig {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [string]$McpUrl,
        [string]$ConfigDirectory = (Join-Path ([Environment]::GetFolderPath("UserProfile")) ".cursor")
    )

    Assert-McpUrl $McpUrl

    $configPath = Join-Path $ConfigDirectory "mcp.json"
    $config = Read-CursorConfig -ConfigPath $configPath
    ...
    if ($null -eq $mcpServersProperty) {
        ...
        if ($mcpServers -isnot [System.Management.Automation.PSCustomObject]) {
            throw "Cursor MCP configuration mcpServers must be a JSON object: $configPath"
        }
    }
    ...
    Write-Output "Configured Cursor MCP in $configPath"
}
...
function Invoke-Onboarding {
    ...
    Write-Output "Onboarding complete. Open Cursor, then run 'cursor-agent mcp login gitlab_deployment' to authorize GitLab in the browser."
}
